...
import { updateElementRow } from '../utils/elementsDataset'
...
/** Formulaire d'édition admin d'un territoire (periodic_elements) : un champ texte par
 *  colonne incluse, sauf le sujet et l'article — renommer un territoire sortirait du périmètre
 *  (elements.i18n.ts reste indexé par le nom exact). Valeurs brutes (non traduites) :
...
export function FicheEditForm({ row, schema, onSaved, onCancel }: FicheEditFormProps) {
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      ...
      await updateElementRow(canonical, values)
      onSaved({ ...row, ...values })
    } catch {
