...
const SESSION_KEY = 'periodic-quiz:guest-pass'
...
export async function checkGuestPass(token: string): Promise<boolean> {
  const { data, error } = await supabase.rpc('check_periodic_guest_pass', { p_token: token })
...
export async function deleteGuestPass(token: string): Promise<void> {
  const { error } = await supabase.from('periodic_guest_passes').delete().eq('token', token)
  if (error) throw error
}
