...
const KEY = 'periodic-quiz:profile'
...
async function pushCloudProfile(userId: string, { pseudo, avatar, theme, locale }: Profile): Promise<void> {
  const { error } = await supabase.from('periodic_profiles').upsert({ id: userId, pseudo, avatar, theme, locale, updated_at: new Date().toISOString() })
  if (error) throw error
}

/** Profil local (toujours dispo hors-ligne), synchronisé avec `periodic_profiles` si connecté :
...
export async function fetchProfile(userId?: string | null): Promise<Profile | null> {
  ...
  try {
    const { data } = await supabase.from('periodic_profiles').select('pseudo,avatar,theme,locale').eq('id', userId).maybeSingle()
    if (data) {
