...
const QUIZ_KEY = 'periodic-quiz:quiz-results'
const QUESTION_KEY = 'periodic-quiz:question-results'
...
async function pushQuizRowsToCloud(userId: string, rows: QuizResultRow[]): Promise<void> {
  ...
  const { error } = await supabase.from('periodic_quiz_results').upsert(rows.map((row) => ({ ...row, user_id: userId })), { onConflict: 'id' })
  if (error) throw error
}

async function fetchCloudQuizRows(userId: string): Promise<QuizResultRow[]> {
  const { data, error } = await supabase
    .from('periodic_quiz_results')
...
async function pushQuestionRowsToCloud(userId: string, rows: QuestionResultRow[]): Promise<void> {
  if (!rows.length) return
  const { error } = await supabase.from('periodic_question_results').upsert(rows.map((row) => ({ ...row, user_id: userId })), { onConflict: 'id' })
  if (error) throw error
}

async function fetchCloudQuestionRows(userId: string): Promise<QuestionResultRow[]> {
  const { data, error } = await supabase
    .from('periodic_question_results')
